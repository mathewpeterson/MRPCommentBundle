<?php
/**
 * This file is part of MRPCommentBundle.
 *
 * (c) Mathew Peterson <mathewpeterson@me.com>
 *
 * This source file is subject to the MIT license that is bundled
 * with this source code in the file LICENSE.
 */
namespace MRP\Bundle\MRPCommentBundle\Exception\Thread;

use MRP\Bundle\MRPCommentBundle\Entity\Thread;
use MRP\Bundle\MRPCommentBundle\Model\ThreadInterface;

class ThreadObjectNotSupported extends \InvalidArgumentException implements ThreadExceptionInterface
{
    public function __construct(ThreadInterface $thread) {
        parent::__construct(sprintf('Thread must be instance of "%s" but "%s" given.', Thread::class, get_class($thread)));
    }
}
